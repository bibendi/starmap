...
- **Инструменты:** административная панель, настройки Solid Queue и LDAP синхронизации, аудит через Audited.
...
```
┌─────────────────────────────────────────────────────────────┐
│ Presentation Layer (Hotwire: Turbo + Stimulus)              │
├─────────────────────────────────────────────────────────────┤
...
- **Frontend:** Hotwire обеспечивает мгновенные обновления без полного SPA, Stimulus контроллеры управляют интерактивностью.
- **Controllers:** ApplicationController инкапсулирует Devise, Pundit, I18n. DashboardsController агрегирует данные для overview/team/personal дашбордов.
...
- **Ядро:** Ruby on Rails 8.1.1, PostgreSQL, Hotwire (Turbo + Stimulus).
- **Веб-сервер:** Puma.
- **Очереди:** Solid Queue (native для Rails 8).
- **Кеширование:** Solid Cache.
- **Тестирование:** RSpec, FactoryBot, Shoulda Matchers, Capybara, Selenium WebDriver, test-prof.
...
- **Команды разработчика:** `bin/dev`, `bin/rails db:migrate`, `bin/rails test`, `bundle exec rspec ...`.
